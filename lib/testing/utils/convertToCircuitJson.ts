 ...
 */
function convertSimplifiedPcbTraceToCircuitJson(simplifiedTrace: SimplifiedPcbTrace): PcbTrace {
  return {
    type: "pcb_trace",
 ...
 */
function convertHdRouteToCircuitJson(hdRoute: HighDensityRoute, id: string, width = 0.1): PcbTrace {
  return {
    type: "pcb_trace",
    pcb_trace_id: id,
    route: hdRoute.route.map((point, index) => {
      ...
      return {
        ...
        y: point.y,
        width,
        layer: point.layer ?? "top", // Default to top if no layer specified
      }
    })
  }
}
...
export function convertToCircuitJson(
  ...
  if ("traces" in input && Array.isArray(input.traces)) {
    ...
    input.traces.forEach(trace => {
      circuitJson.push(convertSimplifiedPcbTraceToCircuitJson(trace) as AnyCircuitElement)
    })
  } 
  // Handle HighDensityRoute[] input
    ...
    input.forEach((route, index) => {
      circuitJson.push(
        convertHdRouteToCircuitJson(route, `trace_${index}`, minTraceWidth) as AnyCircuitElement
      )
    })
  }
