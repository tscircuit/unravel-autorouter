...
import { HighDensityRouteSpatialIndex, HighDensityRoute } from "lib/data-structures/HighDensityRouteSpatialIndex"
...
const drawRoutes = (ctx: CanvasRenderingContext2D, routes: HighDensityRoute[]) => {
...
export default () => {
  ...
  const [conflicts, setConflicts] = useState<Array<{ conflictingRoute: HighDensityRoute; distance: number }>>([])
  ...
  const spatialIndex = useMemo(() => new HighDensityRouteSpatialIndex(routes, 0.5), [routes])
  ...
  useEffect(() => {
    ...
    if (conflicts.length > 0) {
      conflicts.forEach(conflict => {
        ...
        ctx.lineWidth = (conflict.conflictingRoute.traceThickness + MARGIN * 2) * SCALE
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const distToA = Math.sqrt(Math.pow(x - pointA.x, 2) + Math.pow(y - pointA.y, 2))
    const distToB = Math.sqrt(Math.pow(x - pointB.x, 2) + Math.pow(y - pointB.y, 2))
    const distToC = Math.sqrt(Math.pow(x - pointC.x, 2) + Math.pow(y - pointC.y, 2))
    ...
    <div className="p-4">
      ...
      <div className="mb-4">
        <p className="text-sm mb-2">
          Drag points A and B to test segment conflicts, or point C to test point conflicts.
          Current mode: <strong>{testingMode === 'segment' ? 'Segment Testing' : 'Point Testing'}</strong>
        </p>
        <button 
        ...
        <div className="flex-1">
          ...
          <div className="border border-gray-300 rounded p-4 bg-gray-50 h-[400px] overflow-auto">
            ...
            {conflicts.map((conflict, index) => (
              <div key={index} className="mt-3 p-2 border-t border-gray-300">
                <p><strong>Route:</strong> {conflict.conflictingRoute.connectionName}</p>
                <p><strong>Distance:</strong> {conflict.distance.toFixed(4)}mm</p>
                <p><strong>Trace Width:</strong> {conflict.conflictingRoute.traceThickness}mm</p>
                <p><strong>Via Diameter:</strong> {conflict.conflictingRoute.viaDiameter}mm</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 p-3 bg-gray-100 rounded text-sm">
            ...
            <ul className="list-disc pl-5">
              ...
              <li>Board Size: {BOARD_WIDTH}mm × {BOARD_HEIGHT}mm</li>
            </ul>
          </div>
        </div>
      </div>
